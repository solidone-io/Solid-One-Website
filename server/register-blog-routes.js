...
import { createBlogPost, deleteBlogPost, findBlogPostBySlug, getBlogPostById, getPublishedBlogPostBySlug, listBlogPosts, listPublishedBlogPosts, updateBlogPost, } from "./blog-store.js";
...
function postValidationError(parsed) {
    return parsed.error.issues.map((i) => i.message).join(" ") || "Invalid post data.";
}
export function registerBlogRoutes(app, { requireAdmin, uploadsDir }) {
    const upload = multer({
    ...
    app.get("/api/blog/:slug", asyncRoute(async (req, res) => {
    ...
    app.get("/api/admin/blog", requireAdmin, asyncRoute(async (_req, res) => {
        res.json({ posts: (await listBlogPosts()).map(serializePost) });
    }));
    app.get("/api/admin/blog/:id", requireAdmin, asyncRoute(async (req, res) => {
    ...
    app.post("/api/admin/blog", requireAdmin, asyncRoute(async (req, res) => {
    ...
    app.put("/api/admin/blog/:id", requireAdmin, asyncRoute(async (req, res) => {
        const id = Number(req.params.id);
        if (!Number.isInteger(id) || id < 1) {
            res.status(400).json({ error: "Invalid id." });
            return;
    ...
    app.delete("/api/admin/blog/:id", requireAdmin, asyncRoute(async (req, res) => {
        const id = Number(req.params.id);
        if (!Number.isInteger(id) || id < 1) {
            res.status(400).json({ error: "Invalid id." });
            return;
