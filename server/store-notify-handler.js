import { z } from "zod";
import { addStoreNotifySignup, deleteStoreNotifySignup, listStoreNotifySignups, } from "./store-notify-store.js";
const emailSchema = z.string().trim().email().max(320);
const storePlatformSchema = z.enum(["apple", "google"]);
export async function handleStoreNotifyPost(body) {
    const parsedEmail = emailSchema.safeParse(body && typeof body === "object" && "email" in body ? body.email : undefined);
    const parsedPlatform = storePlatformSchema.safeParse(body && typeof body === "object" && "platform" in body ? body.platform : undefined);
...
export async function handleStoreNotifyList() {
    return { status: 200, json: { signups: await listStoreNotifySignups() } };
}
export async function handleStoreNotifyDelete(id) {
...
export function isAdminAuthorized(authHeader, adminPassword) {
